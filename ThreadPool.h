...
/**
 * This thread pool starts by working in "fast mode", if a thread spends too much time without getting work it will
 * sleep on a condition variable and reawaken to "fast mode" again when it gets more work
 */
class ThreadPool
{
public:
	/**
	 *
	 * @param numThreads Number of threads in the thread pool. By default it takes a hint of from the implementation,
	 * if the value isn't well defined or computable, it defaults to 2.
	 * @param waitForAllTasksToFinish The constructor will wait until all the work on the queue has been processed before
	 * it shuts down
	 */
	explicit ThreadPool(unsigned numThreads = std::thread::hardware_concurrency(), bool waitForAllTasksToFinish = false)
		:
		mDone{false},
	 ...
	 * @return A future that can be used to check the value once it's been processed by the thread queue
	 */
	template<typename FunctionType>
	std::future<typename std::result_of<FunctionType()>::type> submit(FunctionType fn)
	{
...
private:
	...
	bool mDone;
	/// True if the worker threads should wait until all the tasks have been processed
	...
	/// This condition is used to signal to the worker threads if an event has happened
	std::condition_variable mWaitForTasks;
	/// The thread loop will yield execution until this maximum was reached, then it "sleeps" by waiting on a CV
